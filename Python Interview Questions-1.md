...
**[Image on Page 12]:**
A flowchart illustrating the logic of a break statement.
- The flow starts and enters a loop.
- A condition within the loop is checked.
- If the condition is false, the loop body continues to execute, and the loop continues.
- If the condition is true, the 'break' statement is executed.
- The flow immediately exits the loop and proceeds to the code following the loop.
- The flowchart ends.
...
```
**Local Variables:**
- Any variable declared inside a a function is known as a local variable. This variable is present in the local space and not in the global space.
...
class Employee_1:
    def __init__(self, name, age,salary):
        self.name = name
        self.age = age
        self.salary = salary # The original PDF had 20000, but the example uses the parameter
...
### 59) Mention the inheritance styles in Django.
